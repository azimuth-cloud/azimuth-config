...
#
# Heavily based on:
# https://opendev.org/openstack/magnum-capi-helm/src/branch/master/devstack/contrib/new-devstack.sh
...
# Create some Azimuth-shaped OpenStack resources
openstack flavor create --ram 8192 --disk 40 --vcpus 2 azimuth.seed
openstack flavor create --ram 4096 --disk 20 --vcpus 2 azimuth.worker
openstack network set --tag portal-internal private

# Remove quota limits on target project
openstack quota set --cores -1 --instances -1 --ram -1 --volumes -1 demo

# Create an app cred for deployment
source /opt/stack/openrc
cat << EOF > clouds.yaml 
clouds:
  openstack:
    auth:
      auth_url: http://$(hostname -I | awk '{print $1}')/identity
      application_credential_id: $(openstack application credential create --secret devstack azimuth-test -f json | jq .id)
      application_credential_secret: "devstack"
