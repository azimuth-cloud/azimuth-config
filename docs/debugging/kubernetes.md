...
## Retrieving a tenant cluster's kubeconfig

The kubeconfigs for all tenant clusters are stored as secrets. First, you need
to find the name and namespace of your cluster. This can be seen from the list
of clusters:
...
```command  title="On the K3s node, targetting the HA cluster if deployed"
$ kubectl -n <namespace> get secret <clustername>-kubeconfig -o json | jq -r '.data.value' | base64 -d > kubeconfig-tenant.yaml
```
