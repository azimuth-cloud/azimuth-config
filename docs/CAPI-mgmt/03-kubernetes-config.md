...
The clusters deployed by the Cluster API driver make use of the Ubuntu Kubernetes images built in the
[azimuth-images repository](https://github.com/azimuth-cloud/azimuth-images) and then use
[capi-helm-charts](https://github.com/azimuth-cloud/capi-helm-charts) to provide the Helm charts
which define the clusters based on these images. Between them, these two repositories have CI jobs
that regularly build and test images and Helm charts for the latest Kubernetes versions. It is
therefore important to update the cluster templates on each cloud regularly to make use of these new
releases.

<!-- prettier-ignore-start -->
!!! note
These templates are a tested set against the specific CAPI management cluster release. As such, you
should make sure to update your CAPI management cluster to the latest release before updating to the
latest templates.
...
<!-- prettier-ignore-start -->
!!! tip
    If you do not have a Docker Hub mirror available, one can be deployed as part of a deployment.
    ...
    This mirror **cannot** be used for the HA cluster as it is deployed on that cluster.
<!-- prettier-ignore-end -->

## Multiple external networks

In the case where multiple external networks are available, you must tell the HA cluster which one
to use:
