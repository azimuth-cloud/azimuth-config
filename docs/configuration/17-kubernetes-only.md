...
- k8s only is an environment/set of features designed to allow azimuth to run WITHOUT openstack on any kubernetes cluster
...
- the host VM is Ubuntu 22.04-24.04 or similar
...
- your tenancy can be managed using CD through `flux`, which will read a repo and then apply the config files there to the cluster. 
...
- the repo also includes a setup script that automates setting up a new tenancy, pushes the files to your repo and then enables flux to track that repo

```
#clone the tennancy config repo on a machine that has a kubeconfig for the cluster
git clone  https://github.com/azimuth-cloud/azimuth-tenant-config
cd azimuth-tenant-config

# Run the setup script (for a more detailed explanation of what the script is doing see the tennancy repo readme) 
